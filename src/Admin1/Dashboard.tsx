...
const Image = ({ src, alt, fill, sizes, priority, className, ...props }: any) => {
  return <img src={src} alt={alt} className={`${className} ${fill ? 'w-full h-full' : ''}`} {...props} />;
}
...
const getIconComponent = (iconName: string) => {
  const iconMap: any = {
    Briefcase, Layers, GraduationCap, Brain, BarChart, Database,
    ShoppingCart, Globe, Server, Blocks, Code, Smartphone,
    Eye, EyeOff, Activity, Award, TrendingUp, Users, Calendar, MapPin,
    Plus, Edit, Trash2, Search, X, Save, Check, LayoutDashboard, Filter,
    Download, Upload, Settings, Star, ExternalLink, Play, Package,
    ArrowRight, Sparkles, Tag, DollarSign, BarChart3, Cpu, Target, CheckCircle, AlertCircle, Clock
  };
                      ...
                      <CardContent className="p-6">
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <p className="font-medium">{categoryLabels[project.category] || project.category}</p>
                          </div>
                        </div>
...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (!access) {
        ...
        // Optionally redirect here or let the user handle it
        return;
      }
      ...
      if (teamRes.ok) {
        ...
        console.error(`Team fetch failed: ${teamRes.status}`);
      }
      ...
      if (galleryRes.ok) {
        ...
        console.error(`Gallery fetch failed: ${galleryRes.status}`);
      }
      ...
      alert("An error occurred while fetching dashboard data. Check console for details.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSyncAll = async () => {
    if (!window.confirm("This will sync all data (including gallery images) from the live server. Continue?")) return;

    setLoading(true);
    try {
      // 1. Sync Gallery
      try {
        await fetch(`${API_URL}/api/gallery/sync/`, {
          method: "POST",
  ...
  const updateProject = async (id: string, updatedData: Partial<Project>) => {
    try {
      ...
      Object.keys(updatedData).forEach((key) => {
        ...
        if (key === "icon") {
          formData.append("icon", String(value));
          return;
        }
          ...
          try {
            if (Array.isArray(value)) {
              ...
              try {
                ...
                if (Array.isArray(parsed)) statsValue = parsed;
                else if (typeof parsed === 'object' && parsed) {
                  statsValue = Object.entries(parsed).map(([k, v]) => ({ label: k, value: String(v) }));
                }
              } catch { statsValue = []; }
            } else if (typeof value === 'object' && value) {
              statsValue = Object.entries(value as Record<string, any>).map(([k, v]) => ({ label: k, value: String(v) }));
            }
          } catch (e) {
      ...
      if (res.ok) {
        ...
        try {
          const json = JSON.parse(text);
          if (json && typeof json === 'object' && Object.keys(json).length === 0) {
            errDetail = `Server returned empty error (Status ${res.status}). Check backend logs.`;
          } else {
        ...
        if (!errDetail) errDetail = `Status: ${res.status} ${res.statusText}`;

        console.error("❌ Parsed error detail:", errDetail);
        alert(`Failed to update project: ${errDetail}`);
      }
    } catch (err) {
      console.error("❌ Error updating project:", err);
      alert(`Error updating project: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
  ...
  const updateGalleryItem = async (id, updatedData) => {
    try {
      ...
      console.log(`Updating gallery item ${id} at ${API_URL}/api/gallery/${id}/`);
      ...
      if (res.ok) {
        ...
        try {
          ...
          alert(`Failed to update gallery item (Status ${res.status}): ${text}`);
        }
      }
    } catch (err) {
  ...
  const ProjectModal = ({
        ...
        return {
          ...
          testimonial: p.testimonial_name
            ? {
              ...
              image: p.testimonial_image || "",
              quote: p.testimonial_quote || "",
              rating: p.testimonial_rating || 5,
            }
            : undefined,
          technologies: Array.isArray(project.technologies)
            ? project.technologies.join(", ")
    ...
    useEffect(() => {
      ...
      setLocalProject({
        ...project,
        testimonial: (project as any).testimonial_name
          ? {
            name: (project as any).testimonial_name,
            role: (project as any).testimonial_role || "",
            image: (project as any).testimonial_image || "",
            quote: (project as any).testimonial_quote || "",
            rating: (project as any).testimonial_rating || 5,
          }
          : undefined,
        technologies: Array.isArray(project.technologies)
          ? project.technologies.join(", ")
    ...
    const handleSave = () => {
      ...
      // Ensure icon is passed
      backendData.icon = localProject.icon || "Briefcase";
        ...
        >
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 md:p-6 mb-6">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={handleSyncAll} variant="outline" size="sm" className="text-xs border-dashed border-blue-300 text-blue-600 hover:bg-blue-50">
                  <Activity className="w-3 h-3 mr-1" />
                  Sync All
                </Button>
                <Button onClick={handleExport} variant="outline" size="sm" className="text-xs">
                  <Download className="w-3 h-3 mr-1" />
                  Export
                </Button>
                <Button onClick={() => alert("Settings panel coming soon!")} variant="outline" size="sm" className="text-xs">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="text-xs">
                          {filteredTeamMembers.length} Result
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-6">
                  {filteredProjects.map(project => {
                    const iconMap: any = { Briefcase, Layers, GraduationCap, Brain, BarChart, Database, ShoppingCart, Globe, Server, Blocks, Code, Smartphone };
                    const ProjectIcon = iconMap[project.icon] || Briefcase;
                        ...
                        <div className="flex-1 p-6 flex flex-col justify-between">
                          <div>
                            <div className="flex justify-between items-start mb-2">
                              <h3 onClick={() => setViewingProject(project)} className="text-xl font-bold text-gray-900 group-hover:text-primary transition-colors cursor-pointer">{project.title}</h3>
                              {/* Icons Area (Edu, DB representations if available, or just status/meta) */}
                              <div className="flex items-center gap-2 text-gray-400">
                                {project.client && <ProjectIcon className="w-4 h-4" title={`Client: ${project.client}`} />}
